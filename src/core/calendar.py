...
from src.core.tasks import add_tasks
...
def sync_calendar_events(max_results=10):
    ...
    for event in events:
        ...
        # Get deadline (support both 'dateTime' for specific time and 'date' for all-day)
        deadline = start.get("dateTime", start.get("date"))

        try:
            # We treat the event start time as the 'deadline'
